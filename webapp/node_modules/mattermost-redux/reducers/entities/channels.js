...
function channels(state, action) {
    ...
    switch (action.type) {
        ...
        case action_types_1.PostTypes.RECEIVED_NEW_POST: {
            var _r = action.data, channel_id = _r.channel_id, create_at = _r.create_at; //eslint-disable-line @typescript-eslint/camelcase
...
function myMembers(state, action) {
    var _a, e_2, _b, _c, _d, _e, _f, _g, _h, _j;
    if (state === void 0) { state = {}; }
    switch (action.type) {
        ...
        case action_types_1.ChannelTypes.INCREMENT_UNREAD_MSG_COUNT: {
            var _m = action.data, channelId = _m.channelId, amount = _m.amount, onlyMentions = _m.onlyMentions;
            ...
            return tslib_1.__assign(tslib_1.__assign({}, state), (_d = {}, _d[channelId] = tslib_1.__assign(tslib_1.__assign({}, member), { msg_count: member.msg_count + amount }), _d));
        }
        case action_types_1.ChannelTypes.DECREMENT_UNREAD_MSG_COUNT: {
            var _o = action.data, channelId = _o.channelId, amount = _o.amount;
            var member = state[channelId];
            if (!member) {
                // Don't keep track of unread posts until we've loaded the actual channel member
                return state;
        ...
        case action_types_1.ChannelTypes.INCREMENT_UNREAD_MENTION_COUNT: {
            var _p = action.data, channelId = _p.channelId, amount = _p.amount;
            var member = state[channelId];
            if (!member) {
                // Don't keep track of unread posts until we've loaded the actual channel member
                return state;
            }
            return tslib_1.__assign(tslib_1.__assign({}, state), (_f = {}, _f[channelId] = tslib_1.__assign(tslib_1.__assign({}, member), { mention_count: member.mention_count + amount }), _f));
        }
        case action_types_1.ChannelTypes.DECREMENT_UNREAD_MENTION_COUNT: {
            ...
            return tslib_1.__assign(tslib_1.__assign({}, state), (_g = {}, _g[channelId] = tslib_1.__assign(tslib_1.__assign({}, member), { mention_count: Math.max(member.mention_count - amount, 0) }), _g));
        }
        case action_types_1.ChannelTypes.RECEIVED_LAST_VIEWED_AT: {
            ...
            return tslib_1.__assign(tslib_1.__assign({}, state), (_h = {}, _h[action.data.channel_id] = member, _h));
        }
        case action_types_1.ChannelTypes.LEAVE_CHANNEL: {
            var nextState = tslib_1.__assign({}, state);
            if (action.data) {
        ...
        case action_types_1.ChannelTypes.POST_UNREAD_SUCCESS: {
            ...
            return tslib_1.__assign(tslib_1.__assign({}, state), (_j = {}, _j[data.channelId] = tslib_1.__assign(tslib_1.__assign({}, channelState), { msg_count: data.msgCount, mention_count: data.mentionCount, last_viewed_at: data.lastViewedAt }), _j));
        }
        case action_types_1.ChannelTypes.RECEIVED_MY_CHANNELS_WITH_MEMBERS: { // Used by the mobile app
            var nextState_3 = tslib_1.__assign({}, state);
            var current = Object.values(nextState_3);
            var _r = action.data, sync = _r.sync, channelMembers_1 = _r.channelMembers;
